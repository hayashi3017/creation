...
- Diagram API は list の `GET /api/diagrams` だけ resource-oriented 側に揃っていて、write 系は `POST /api/diagrams/create`, `PATCH /api/diagrams/update/{diagram_id}`, `DELETE /api/diagrams/delete/{diagram_id}` の action-style な形が残っている。resource-oriented な path へ寄せるか、この形を正式仕様にするかを決めた方が API の一貫性が上がる。See `docs/rfc/0001-resource-oriented-api-shape.md`.

## Person API

- Person API は `GET /api/persons` が body に依存し、write 系は `POST /api/persons/create`, `PATCH /api/persons/update/{entity_id}`, `DELETE /api/persons/delete/{entity_id}` の action-style な形になっている。resource-oriented な path と parameter policy に揃えるか、この形を正式仕様にするかを決めた方が API の一貫性が上がる。See `docs/rfc/0001-resource-oriented-api-shape.md`.
...
- RFC 0010 で公開 identifier 名は explicit に揃ったが、create 系 endpoint はまだ `{ "response": "ok" }` だけを返している。作成直後の resource id を返すかどうかは別途決めた方がよい。See `docs/rfc/0010-explicit-id-fields-in-public-api.md`.
- User API の timestamp は `createdAt` / `updatedAt` のままなので、将来 response naming を全面的に揃えるなら `snake_case` へ寄せるか現状維持かを決めた方がよい。See `docs/rfc/0010-explicit-id-fields-in-public-api.md`.
...
- RFC 0015 の初期実装では `GET /api/genealogy/world/{world_id}` は full overview を返し、`center_entity_id` / `ancestor_depth` / `descendant_depth` は request contract として受け取るだけにしている。center-relative metadata の exact rule と response contract は RFC 0024 の共通 graph payload contract に沿って詰める必要がある。See `docs/rfc/0015-genealogy-overview-api.md` and `docs/rfc/0024-genealogy-graph-payload-contract.md`.
